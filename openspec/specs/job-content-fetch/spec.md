...
- **Test coverage gap — since resolved.** At initial (retroactive) authoring of this spec, `worker/jobs/fetch-job-content.mjs`, `worker/shared/fetch-page.mjs`, `isHostAllowed`, the `ingest-email.mjs` enqueue call, and `AddManual`'s enqueue-gate behavior had zero test files. This was closed by a follow-up `sdd-apply` pass (commit `2a24a3e`): `worker/tests/jobs/fetch-job-content.test.mjs` (happy path + 4 failure branches), direct `isHostAllowed` coverage in `worker/tests/lib/url-normalize.test.mjs`, `ingest-email.mjs`'s enqueue branch in `worker/tests/jobs/ingest-email.test.mjs`, and `AddManual`'s gate via a DB-backed integration test (`api/internal/jobs/addmanual_enqueue_integration_test.go`, `TestAddManual_EnqueueGate`). Re-verified PASS. Residual, non-blocking gap: `fetchPageText` itself (a thin Playwright wrapper) still has no direct unit test, only coverage via its caller's mock boundary.
