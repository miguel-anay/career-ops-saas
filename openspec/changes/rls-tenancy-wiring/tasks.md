...
| T-122 | verify | [ ] Run `make test-rls` — confirm T-118/T-121 pass AND every pre-existing pgTAP assertion (per-table cross-tenant invisibility, FORCE RLS flags) is still green with no test changes required — Req 4 scenario "pgTAP RLS tests stay green after the NULLIF migration" — **deferred to orchestrator** (requires Docker, not run by sdd-apply per instructions) | n/a (verification step) |
