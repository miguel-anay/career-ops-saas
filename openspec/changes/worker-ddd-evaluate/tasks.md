...
- [x] T-170 [impl] Create `worker/adapters/_ports.js` — JSDoc-only `EvaluatorPort {evaluate(userId, jobId): Promise<string>}` and `EvaluationRepository {save(userId, jobId, evaluation): Promise<void>}`, mirroring `worker/providers/_types.js` style (no runtime export besides `export {}`).
- [x] T-171 [test] Create `worker/tests/application/evaluate-job.test.mjs`: build a fake evaluator (`{ evaluate: vi.fn().mockResolvedValue(rawText) }`) and a spy repository (`{ save: vi.fn() }`), construct `new EvaluateJob({evaluator, repository})`, call `.run({userId, jobId})`, assert `repository.save` was called once with `(userId, jobId, evaluationInstance)` where `evaluationInstance.score`/`.blocks`/`.isParseError` match the parsed rawText. Cover happy path AND parse-error path (empty rawText). Verify: fails (EvaluateJob.mjs doesn't exist).
